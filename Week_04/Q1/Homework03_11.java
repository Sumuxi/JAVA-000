...
//        int result = sum(); //这是得到的返回值
		...
		try {
			final Method method = Homework03_06.class.getDeclaredMethod("sum");
			method.setAccessible(true);
			MyThread t = new MyThread(method);
			t.start();
			
			//利用线程的synchronized的同步机制，通过Thread.sleep(1)触发操作系统立刻进行一次CPU调度,期望主线程让出CPU，线程T先进入同步块执行，线程T结束后主线程再进入同步块，即可拿到结果
