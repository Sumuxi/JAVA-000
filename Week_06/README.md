...
用户表：主要包含一些很基本的信息，当然也包含位置区域信息，考虑基于地理位置进行服务。

商品表：由于对电商的商品不太了解，凭个人的生活经验和感觉设置的字段。

订单表：主要包含订单状态、支付信息、快递信息、发票信息以及关联的商品快照信息。
...
CREATE TABLE `ums_member` (
  ...
  `city` varchar(64) DEFAULT NULL COMMENT '所在城市',
	`district` varchar(64) DEFAULT NULL COMMENT '所在区/县/州',
...
CREATE TABLE `pms_product` (
  ...
  `preview_status` int(1) DEFAULT NULL COMMENT '是否为预售商品：0->不是；1->是',
...
CREATE TABLE `oms_order_item` (
  ...
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=1 DEFAULT CHARSET=utf8 COMMENT='订单中所包含的商品快照';
```
